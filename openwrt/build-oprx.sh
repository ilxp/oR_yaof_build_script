...
if [ "$isCN" = "CN" ]; then
	...
	export mirror=https://raw.githubusercontent.com/ilxp/oR_yaof_build_script/main
else
...
if [ -d openwrt ]; then
    ...
    curl -Os $mirror/openwrt/data/patches/key.tar.gz && tar zxf key.tar.gz && rm -f key.tar.gz
else
...
curl -sO $mirror/openwrt/SCRIPTS/02_prepare_package-oprx.sh
curl -sO $mirror/openwrt/SCRIPTS/X86/02_target_only-oprx.sh
curl -sO $mirror/openwrt/SCRIPTS/04_remove_upx.sh
...
if [ "$platform" = "x86_64" ]; then
    ...
    curl -s $mirror/openwrt/SEED/X86/oprx-config.seed > .config
	#curl -s $mirror/openwrt/SEED/X86/oprx-config-small.seed > .config
elif [ "$platform" = "bcm53xx" ]; then
    ...
    curl -s $mirror/openwrt/23-config-musl-armsr-armv8 > .config
else
    curl -s $mirror/openwrt/23-config-musl-r4s > .config
fi

curl -sO $mirror/openwrt/SCRIPTS/03_convert_translation.sh
curl -sO $mirror/openwrt/SCRIPTS/05_create_acl_for_luci.sh
#curl -sO $mirror/openwrt/SCRIPTS/08_fix_permissions.sh
chmod 0755 *sh
...
if [ "$BUILD_FAST" = "y" ]; then
    ...
    if [ "$PLATFORM_ID" = "platform:el9" ]; then
        ...
        TOOLCHAIN_URL=https://"$github_proxy"github.com/ilxp/openwrt_caches6.6/releases/download/openwrt-24.10
    fi
...
#curl -sO $mirror/openwrt/SCRIPTS/07_fw3-oprx.sh
#chmod 0755 *sh
#bash 07_fw3-oprx.sh
...
EOF
	...
	#sha5_efi4=$(egrep -o '[a-z0-9]+' <<< ${SHA256_efi} | cut -c1-5)  #获取前5位
	...
	#rename -v "s/openwrt-x86-64-generic-ext4-combined-efi/OprX-openwrt-x86_64-oR-$OP_VERSION-Fext4-uefi-$sha5_efi4/" bin/targets/x86/64*/*.gz || true  #能成功
