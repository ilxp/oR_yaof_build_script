...
#ReV_Date="v$(curl -s https://api.github.com/repos/openwrt/openwrt/tags | grep -Eo "v24.10.+[0-9\.]" | head -n 1 | sed 's/v//g')"
...
sed -i "s/DISTRIB_DESCRIPTION.*/DISTRIB_DESCRIPTION='OprX oR%C @${Build_DATE}'/g" package/base-files/files/etc/openwrt_release

sed -i "/OPENWRT_RELEASE/d" package/base-files/files/usr/lib/os-release
sed -i "/OPENWRT_DEVICE_REVISION/aOPENWRT_RELEASE=\"OprX oR%C @${Build_DATE}\"" package/base-files/files/usr/lib/os-release
